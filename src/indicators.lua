...
local function Indicators.MA(Period,Metod,VType,round,ds) --Moving Average ("MA")
local t_SMA = F_SMA()
local t_EMA = F_EMA()
local t_VMA = F_VMA()
local t_SMMA = F_SMMA()
...
return function(I, ds)
	...
	local P = (Period or 9)
	local M = (Metod or "EMA")
	local VT = (VType or "Close")
	local R = (round or "off")
	if M == "SMA" then
...
local function rounding(num, round) 
...
local function Value(I,VType,ds) 
local Out = nil
